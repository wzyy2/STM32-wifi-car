...
void USART_Config()
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;               //选择GPIOB的6脚
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;          //设置选中的引脚工作模式，IN：输入模式；OUT：输出模式；AF：复用模式；AN：模拟模式
	GPIO_Init(GPIOD, &GPIO_InitStructure);        //初始化所设置的GPIOD参数
  //RX
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;                //选择GPIOB的7脚
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;          //设置选中的引脚工作模式，IN：输入模式；OUT：输出模式；AF：复用模式；AN：模拟模式
...
/*****************************************/
  ...
  GPIO_PinAFConfig(GPIOD , GPIO_PinSource9 , GPIO_AF_USART3);
	...
	if(flag)
USART_InitStructure.USART_BaudRate =2250000L;
  // flag=1;
	else
	{
...
flag=1;
	...
	//串口1参数设置                                     //波特率：115200、57600、38400、9600、4800、2400、1200
