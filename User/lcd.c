...
{ 										  
  ...
  FSMC_NORSRAMTimingInitTypeDef  readWriteTiming; 
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;												  //设置管脚为Pin_5,根据自己液晶接法设置
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;										  //设置上/下拉电阻为无上拉下拉电阻		
    GPIO_Init(GPIOE, &GPIO_InitStructure);													  //初始化GPIOB的Pin_5
    /* 配置基于FSMC的液晶屏的数据线
  ...
  GPIO_PinAFConfig(GPIOE,GPIO_PinSource3,GPIO_AF_FSMC);		//配置GPIOD的Pin11复用连接到FSMC
